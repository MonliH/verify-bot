...
async def set_state(snowflake: int, status: Status):
    match status:
        case SentVerification(email, code):
            await state_collection.insert_one({"snowflake": snowflake, "status": {"type": "sent", "email": email, "code": code}})
        case Verified:
            await state_collection.insert_one({"snowflake": snowflake, "status": {"type": "verified"}})
